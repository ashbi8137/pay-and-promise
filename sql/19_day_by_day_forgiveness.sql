-- Day-by-Day Forgiveness Logic
-- This updates the settlement function to REFUND penalties if everyone failed for that day.
...
begin
    ...
    -- Insert Auto-Fails for missing users
    if v_missing_participants is not null then
        foreach v_missing_user in array v_missing_participants loop
            ...
            -- Apply provisional penalty (might be refunded later)
    ...
    -- E. Check Completeness (Wait until everyone has Verified or Rejected status)
    ...
    -- === [NEW LOGIC: DAY-BY-DAY FORGIVENESS] ===
    -- If NO ONE won (Winners = 0), then we REFUND everyone for this day.
    if v_winners_count = 0 then
        -- Find everyone who failed today (which is everyone)
        for v_loser_id in 
            select user_id from public.promise_submissions 
            where promise_id = p_promise_id and date = p_date
        loop
             -- Refund the penalty (Clean way: Insert a positive adjustment or 'refund')
             -- Just inserting a 'refund' type entry to balance the ledger
             select stake_per_day into v_stake from public.promises where id = p_promise_id;
             
             insert into public.ledger (promise_id, user_id, amount, type, description)
             values (p_promise_id, v_loser_id, v_stake, 'refund', 'Day cancelled: Everyone failed');
        end loop;
        
        -- Settle with 0 pool
        insert into public.daily_settlements (promise_id, date, total_pool, winners_count, amount_per_winner)
        values (p_promise_id, p_date, 0, 0, 0);
        
        return;
    end if;
    -- ===========================================

    -- 2. Standard Settlement (If at least 1 person won)
    select stake_per_day into v_stake from public.promises where id = p_promise_id;
