-- =================================================================
-- 58_update_deadline_auto_resolve.sql
-- AUTO-RESOLVE CRON: Settle days for users who missed the deadline
-- AND auto-verify users whose peers did not review their proof.
-- =================================================================
...
declare
    ...
    v_stake numeric;
begin
    -- 1. Auto-verify pending submissions that peers neglected to vote on
    -- This ensures the uploader isn't penalized for their peers' inactivity.
    update public.promise_submissions ps
    set status = 'verified'
    where ps.date = v_yesterday
    and ps.status = 'pending'
    and exists (
        select 1 from public.promises p 
        where p.id = ps.promise_id 
        and p.status = 'active'
    );

    -- 2. Find all active promises that should have been settled yesterday
    for v_promise in
    ...
    loop
        ...
        -- The check_and_finalize_verification RPC will read the 'verified' statuses 
        -- we updated in step 1, and sync them seamlessly to daily_checkins.
